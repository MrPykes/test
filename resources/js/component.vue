<template>
    ...
    <svg xmlns="http://www.w3.org/2000/svg" height="100%"  width="100%" xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="xMidYMin slice" viewBox="0 0 728 400">
        ...
        <text font-family="monospace" font-size="12" fill="#1d1f20">
                ...
                const getText = async() => {
                    await axios.get('/api/random').then(result => {
                       var text = result.data;
                        var characters =  text.map(elem => {
                                return elem.values;
                            });           
                            const t = characters.toString();
                                ...
                                const path = getPath({x:400,y:230}, 0, 15, 0, 6*360, 10);
